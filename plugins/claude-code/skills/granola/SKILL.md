...
Use the `granola` CLI to read meeting notes, AI-generated summaries, and transcripts from [Granola.ai](https://granola.ai). The CLI is read-only — Granola's public API exposes no write endpoints. The agent acts as the user via a personal API key the user generated in the Granola desktop app.
...
- [`./references/notes.md`](./references/notes.md) — `granola notes list | read`. List filters, summary vs transcript, output envelope, speaker labels.
- [`./references/folders.md`](./references/folders.md) — `granola folders list`. Folder hierarchy via `parent_folder_id`.

If `granola whoami` exits 1 with `GRANOLA_API_KEY is not set`, the deployment is missing the env var — that's an operator/deployer fix, not an agent fix. Relay the CLI's stderr message to the user and point them at the [README](https://github.com/alavida-ai/granola-plugin#claude-code-deployment).
...
2. **The CLI is read-only.** There are no write endpoints. Do not promise to "save", "tag", or "edit" notes — those happen in the Granola desktop app.
...
When chaining CLI output into the next agent step, only consume **stdout**. Stderr carries human-readable status, prompts, and errors and is not stable for parsing. With `--json`, stdout is a single JSON object or envelope; without `--json`, stdout is plain text (one record per line for lists).
