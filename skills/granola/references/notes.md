...
- **`--select id,title,created_at`** — project to a subset of fields. Reduces token cost for downstream LLM steps.
...
- **`--text`** — print plain-text summary (`summary_text`). Default is `summary_markdown`, falling back to text if markdown is null.
...
| `folder_membership` | `[{id, name, parent_folder_id, ...}, ...]` |
| `summary_text` | plain-text summary |
| `summary_markdown` | markdown-formatted summary (or null) |
| `transcript` | `[{speaker: {source, diarization_label}, text, start_time, end_time}, ...]` — only present when `--transcript` is set |
...
### How the CLI renders transcripts

Priority order, per segment:
1. **`speaker.diarization_label` populated** → use that label verbatim. (iPhone face-to-face meetings.)
2. **Both `microphone` AND `speaker` sources present in the transcript** → label segments as `Me:` / `Them:`. (Desktop virtual meetings — Meet/Teams/Zoom.)
3. **Only `microphone` source** → render plain text, no labels. (Desktop in-person meetings: a single room mic captures all voices, so `microphone` doesn't mean "you" anymore.)
...
# Action items: read summary as text and let the LLM extract them
granola notes read NOTE_ID --text
